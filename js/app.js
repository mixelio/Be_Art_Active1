(() => {
    ...
    const createSliderObserver = () => {
        const observer = new IntersectionObserver((entries => {
            entries.forEach((entry => {
                if (entry.isIntersecting) {
                    ...
                    console.log(x);
                    console.log("------------------------- нашли ----------------------------");
                    for (let c = 0; c < players.length; c++) if (elements[c].classList.contains("_active")) {
                        console.log(players[c]);
                        players[c].play();
                        players[c].muted = true;
                        players[c].loop = true;
                        players[c].ratio = "16:9";
                    }
                } else {
                    entry.target.classList.remove("_active");
                    console.log("------------------------- пропал ---------------------------");
                    for (let d = 0; d < players.length; d++) if (!elements[d].classList.contains("_active")) {
        ...
        const elements = document.querySelectorAll(".plyr__video-embed");
